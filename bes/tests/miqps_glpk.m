...
%% some allowed exit codes from GLPK
GLP_EITLIM = -8;  %% only this one seems useful to MOST
GLP_ETMLIM = -9;
GLP_EMIPGAP = -14;
...
if mi && eflag == 1 && (~isfield(opt, 'skip_prices') || ~opt.skip_prices)
    ...
    if eflag ~= eflag_
        if eflag_ == GLP_EITLIM
            warning('miqps_glpk: exceeded limit of %d iterations; solution is suboptimal', glpk_opt.itlim);
        elseif eflag_ == GLP_ETMLIM
            warning('miqps_glpk: exceeded limit of %d milliseconds; solution is suboptimal', glpk_opt.tmlim);
        elseif eflag_ == GLP_EMIPGAP
            warning('miqps_glpk: exceeded relative MIP gap tolerance of %e; solution is suboptimal', glpk_opt.mipgap);
        else
