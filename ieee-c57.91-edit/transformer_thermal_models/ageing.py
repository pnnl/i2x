...
def estimate_loss_of_life(T_hs_input,time_input,T_reference=110,B=15000.,A=9.8E-18,nominal_insulation_life=180000,TUK=True):    
    ...
    # equation (1) section 5.2
    per_unit_life = A*np.exp(B/T_hs_input)
    
    # V is the aging acceleration factor (FAA) equation (2) is with TUK
    # not sure where the non TUK solution comes from
    if TUK:
    ...
    # change dt prepend so that non integer time_input possible.
    # dt = np.diff(time_input, prepend=-1)
    dt = np.diff(time_input, prepend=time_input[0]-time_input[1])
    
    # Equivalent aging factor (F EQA) equation (3)
    V_dt = integrate.cumulative_trapezoid(V*dt, initial=V[0])
    # V_dt = np.cumsum(V*dt)
    
    F_eqa = V_dt / integrate.cumulative_trapezoid(dt, initial=dt[0])
    # F_eqa = V_dt/np.cumsum(dt)
    # Equation (4)
...
def solve_estimated_loss_of_life(Transformer,LoadConditions):
    ...
    # these are equation (1) in clause 5.2
    ...
    Transformer.solution["Per Unit Life"] = per_unit_life
