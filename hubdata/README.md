...
We propose fourteen new data items for tracking the outcomes of DER 
...
7.  **Date of Application** \[MM/YYYY\]
8.  **Withdrawal Reason** \[permitting, interconnection costs, site control, environmental, unknown, other, not applicable\]
9.	**Time from Application to Interconnection Agreement** \[days\], or blank if the project is not finished. This measures time taken by the utility to handle the application, including all studies.
10.	**Time from Construction Completion to Permission to Operate** \[days\], or blank if the project is not finished. This measures time taken by the utility for inspection and commissioning.
11.	**Status** \[active, operational, withdrawn, suspended\]: These are the same four status flags used in the LBNL Queued-Up data for BES interconnections.
12.	**Estimated Cost of Studies and Fees** \[$ quoted from the utility\]
13.	**Estimated Cost of System Upgrades** \[$ quoted from the utility\]
14.	**Final Cost of Interconnection** \[$ billed from the utility\]: This captures any difference from the sum of the two estimates.

Most of these items would be readily available to utilities, as items 1-7 
come from the DER application and items 9-14 are generated by the utility.  
Item 8, the reason for withdrawn applications, may not have been disclosed 
to the utility.  Considering the significance of this data to i2X metrics, 
the applicant may be willing to provide this information voluntarily.  

These data items would support metrics on the time and cost of 
interconnections, correlated to location, size, type, and use of advanced 
standards.  In some cases, use of advanced standards can increase the 
ability of the grid to host DER.  The census-based location provides a 
link to demographic data for Energy Equity and Environmental Justice 
(EEEJ) metrics, and it can be aggregated into larger spatial datasets.  
Size, cost, and location data may already be publicly available in some 
jurisdictions or service territories.  Specific project addresses, owner 
names, or developer names are not necessary; the census block group 
location is precise enough for analyzing process metrics.  When a DER 
project is large enough that only one or two could exist in a census block 
group, then items 1-14 could be identifiable to a single project.  Some 
stakeholders may be concerned about that, but this data will be important 
