...
def configure_queue (cfg):
  global HCA_PMAX, HCA_QMAX #, ALT_WIND
...
def add_renewable (mpd, poc, bes_size, ic_cost):
  global bWind
  if bWind:
    genfuel = 'wind'
    gentype = 'WT'
...
def process_renewable_project (poc, mw_size, mpd, G, load_scale, itmax):
...
def process_auction (poc_buses, sys_config):
  base_case = sys_config['base_case']
  load_scale = sys_config['load_scale']
  print ('***********************************************')
...
def process_queue (queue, sys_config):
  base_case, load_scale = configure_queue (sys_config)
  print ('***********************************************')
  print ('Queue Process on {:d} applications for {:s}'.format (len(queue), base_case))
  ...
  for app in queue:
    ...
    mpd, G, cost, hc = process_renewable_project (app['poc'], app['mw'], mpd, G, load_scale, app['itlim'])
    if hc >= app['mw']:
      if cost <= app['costlim']:
        mpd = add_renewable (mpd, app['poc'], app['mw'], cost)
      else:
